...
//停售套餐
    ...
    public R<String> updateStatus(@PathVariable("status") Integer status,@RequestParam("ids") List<Long> ids){
        log.info("status:{},停售启售的ids：{}",status,ids);
        //通过ids批量获取套餐
        List<Setmeal> setmeals = setmealService.listByIds(ids);
        //利用Stream流，设置setmeal的status字段为传过来的status值
        setmeals = setmeals.stream().map(s->{s.setStatus(status);return s;}).collect(Collectors.toList());
        //批量修改
